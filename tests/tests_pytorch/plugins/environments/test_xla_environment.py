...
def test_detect(monkeypatch):
    ...
    monkeypatch.setattr(pl.accelerators.tpu.TPUAccelerator, "is_available", lambda: False)
    assert not XLAEnvironment.detect()

    monkeypatch.setattr(pl.accelerators.tpu.TPUAccelerator, "is_available", lambda: True)
